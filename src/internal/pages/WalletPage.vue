    ...
    <InternalSidebar :title="t('Wallet')" :icon="Wallet" activeKey="wallet">
      <nav class="flex flex-column gap-12px">
        <UiSidebarNavSection :title="t('Activity')">
          <!--
            Wallet now opens the multi-chain panel, which covers the home chain
            as one of its 221 rather than as a screen of its own.

            The two entries below are commented rather than deleted: the panel
            replacing them has not been through a full pass of real
            transactions yet, and the views they point at still work. Delete
            them once it has - not before.
          ...
          -->
          <UiSidebarNavItem :active="currentView === 'cosmos'" @click="currentView = 'cosmos'">
            <Wallet :size="18" />
            <span>{{ t('Wallet') }}</span>
          </UiSidebarNavItem>
    ...
    <main class="flex flex-column flex-1 m-0px min-w-0 overflow-y-auto py-32px px-40px bg-secondary border-radius-0">
      <UiPageHeader :title="getViewTitle()" title-size="20px" title-weight="light" :subtitle="getViewDescription()">
        <template v-if="currentView !== 'dex'" #actions>
          ...
          <UiButton variant="primary" @click="sendTransaction" v-else-if="currentView !== 'cosmos'">
            <Send :size="16" />
            <span>{{ t('Send') }}</span>
          </UiButton>
        </template>
      ...
      <!-- ####### lumen://wallet COSMOS CHAINS PANEL ####### -->
      <CosmosChainsPanel
        v-else-if="currentView === 'cosmos'"
        :address="address"
        :profile-id="activeProfileId"
        @import-profile="openProfileImport"
        @send="openCosmosSendModal"
      />
...
import { describeIbcChannel, fetchIbcTransferChannels } from '../services/ibcChannels';
...
} from '../services/cosmosFees';
import osmosisLogoUrl from '../../img/osmosis.svg';
import { payReminder } from '../services/paymentReminders';
import { formatDecimal, formatDenom as formatDenomValue, truncateMiddle } from '../services/format';
...
import type {
  ...
  DexStatus,
  DexRow
, ContactForm, SendForm, IbcForm, AssetTransferForm, ChainFeeSchedule, CosmosChainSummary, CosmosIbcPeer, FeeTier } from '../../types/walletPage';
...
const currentView = ref<'overview' | 'assets' | 'dex' | 'transactions' | 'addressbook' | 'recurring' | 'cosmos'>('cosmos');
...
/**
 * The fee schedule of the chain being sent on, when it charges one.
 *
 * Null on Lumen, which has no gas market, and null on any chain whose registry
 * entry publishes no fee token - in which case the send is refused rather than
 * broadcast with a guessed price.
 */
const sendFeeSchedule = ref<ChainFeeSchedule | null>(null);

/**
 * How to render the fee token's base amount.
 *
 * Carried explicitly rather than assumed: exponents across the registry run
 * from 6 to 18, and reading an 18 as a 6 renders a fee a trillion times too
 * large. The chain being sent on is the one that knows.
 */
const sendFeeDisplay = ref<{ decimals: number; symbol: string }>({ decimals: 6, symbol: '' });

/** A base-denom fee amount, in the fee token's own units. */
function formatFeeAmount(baseAmount: string): string {
  const raw = Number(baseAmount);
  const { decimals, symbol } = sendFeeDisplay.value;
  if (!Number.isFinite(raw)) return '';
  const value = formatDecimal(raw / 10 ** decimals, { decimals, trimTrailingZeros: true, empty: '0' });
...
/**
 * The exponent of the asset being sent.
 *
 * Deliberately not `sendFeeDisplay.decimals`: the fee is paid in the chain's
 * own token and the asset may be something else entirely - an IBC token held
 * on that chain has its origin's exponent, not its host's.
 */
const sendAssetDecimals = ref(6);
...
const sendFeeTier = computed<FeeTier>(() => {
  const value = String(sendForm.value.gasFee || '').trim();
  return (FEE_TIERS as string[]).includes(value) ? (value as FeeTier) : 'average';
});

/** Priced tiers for the dialog, empty when the chain charges no gas. */
const sendFeeTierOptions = computed(() => {
  const schedule = sendFeeSchedule.value;
  if (!schedule) return [];

  const amounts = feeAmountsByTier(schedule);
  const labels: Record<FeeTier, string> = {
    low: t('Low'),
    average: t('Average'),
    high: t('High')
  };
  return FEE_TIERS.map((tier) => ({
    tier,
    label: labels[tier],
    cost: formatFeeAmount(amounts[tier])
  }));
});

const sendFeeLabel = computed(() => {
  const schedule = sendFeeSchedule.value;
  if (!schedule) return '';
  return formatFeeAmount(computeFeeAmount(schedule, sendFeeTier.value));
});

function setSendFeeTier(tier: FeeTier) {
  sendForm.value.gasFee = tier;
}
...
/**
 * Sends the user to where profiles are actually imported, rather than opening a
 * second key entry here.
 *
 * A Cosmos seed phrase is a Lumen seed phrase - the chains in this panel all
 * share one derivation path - so importing one belongs in profile management
 * with every other key, not in a per-chain corner of the wallet. Two places to
 * enter a recovery phrase means two places to back up, and one of them will be
 * the one the user forgets.
 */
function openProfileImport() {
  // Straight to the recovery-phrase tab: that is what the banner offered, and
  // landing on the file picker would make the user click again to reach it.
  requestProfileImport('manual');
}

/**
 * Opens the send dialog for a chain picked in the Cosmos panel.
 *
 * The dialog, the form and `confirmSend` are reused whole rather than a second
 * send flow being written beside them: this one already handles the password
 * prompt, the two-minute timeout and the unconfirmable-broadcast verdict, and
 * none of that gets better for being duplicated.
 *
 * The fee is loaded before the dialog opens, and a chain that publishes none is
 * refused here rather than failing at broadcast with a guessed gas price.
 */
async function openCosmosSendModal(payload: {
  chain: CosmosChainSummary;
  address: string;
  microAmount: string;
  mode: 'send' | 'ibc';
  ibcPeers: CosmosIbcPeer[];
  /** Set when sending something other than the chain's own token. */
  asset?: { denom: string; symbol: string; decimals: number };
  /** A single route the caller already knows, skipping the channel lookup. */
  ibcRoute?: { channelId: string; portId: string; chainId: string; chainLabel: string; iconUrl: string };
}) {
  const { chain, address: ownerAddress, microAmount, mode, ibcPeers, asset, ibcRoute } = payload;
  ...
  sendFeeSchedule.value = schedule;
  sendFeeDisplay.value = { decimals: chain.decimals, symbol: chain.symbol || chain.denom };
  // The asset's own exponent, which is the origin chain's for an IBC token.
  sendAssetDecimals.value = asset?.decimals ?? chain.decimals;

  void refreshActivities();
  sendAssetContext.value = {
    id: `cosmos-${chain.name}`,
    chainId: chain.chainId,
    chainLabel: chain.prettyName,
    ownerAddress,
    ...
    restEndpoint: chain.rest[0] || '',
    feeDenom: schedule?.denom || chain.denom
  };

  showContactPicker.value = false;
  ibcForm.value = { sourceChannel: '', sourcePort: 'transfer' };
  sendForm.value = { recipient: '', amount: '', gasFee: 'average' };
  sendTargetMode.value = mode === 'ibc' ? 'ibc' : 'lumen';
  showSendModal.value = true;

  if (mode === 'ibc') {
    if (ibcRoute) setSingleIbcRoute(ibcRoute);
    else await loadCosmosIbcChannels(chain, ibcPeers);
  }
}

/**
 * IBC routes out of a Cosmos-panel chain, from the registry rather than the node.
 *
 * `loadIbcChannels` next door asks the connected Lumen node for its own
 * channels, which is right when Lumen is the source and wrong for every other
 * chain - it would offer Lumen's channel ids for a transfer leaving Osmosis.
 *
 * The registry publishes one file per pair with both sides' channel ids, so
 * routes are read per destination. Four at a time: a chain like Osmosis has
 * over 150 registered peers, and opening 150 connections to fill a dropdown is
 * not a dropdown, it is an outage.
 */
/**
 * Offers exactly one route, already chosen and pre-selected.
 *
 * Returning a hosted token to the chain that issued it has one correct channel
 * - the one it arrived through, read off its own denom trace - so presenting a
 * picker would be offering a choice where a wrong answer exists and a right one
 * is already known.
 */
function setSingleIbcRoute(route: {
  channelId: string;
  portId: string;
  chainId: string;
  chainLabel: string;
      ...
      ...describeIbcChannel(
        {
          ...
          counterpartyPortId: route.portId,
          connectionId: '',
          state: 'STATE_OPEN'
        },
        route.chainId
      ),
      label: route.chainLabel,
  ...
  ibcChannelsLoaded.value = true;
  ibcChannelsError.value = '';
  ibcForm.value = { sourceChannel: route.channelId, sourcePort: route.portId };
}

async function loadCosmosIbcChannels(chain: CosmosChainSummary, peers: CosmosIbcPeer[]) {
  ibcChannels.value = [];
  ibcChannelsLoaded.value = false;
  ibcChannelsError.value = '';
  ibcChannelsLoading.value = true;
  ...
  try {
    const queue = [...peers];
    const found: IbcChannelOption[] = [];

    const workers = new Array(Math.min(4, queue.length)).fill(null).map(async () => {
      for (let peer = queue.shift(); peer; peer = queue.shift()) {
        try {
          const raw = await loadIbcPairChannels(chain.name, peer.name);
          for (const entry of raw) {
            // `chainId` stays the raw id: the prefix hints that decide where a
            // transfer lands are derived from it. Only the label is prettified.
            found.push({
              ...describeIbcChannel(entry, peer.chainId || peer.prettyName),
              label: `${peer.prettyName} (${entry.channelId} → ${entry.counterpartyChannelId})`,
              chainLabel: peer.prettyName,
              iconUrl: peer.image
            });
          }
        } catch {
          // One unreachable pair file must not empty the whole list.
        }
      }
    });
    await Promise.all(workers);

    ibcChannels.value = found.sort((a, b) => a.label.localeCompare(b.label));
    ibcChannelsLoaded.value = true;
    if (!ibcChannels.value.length) {
      ibcChannelsError.value = t('No open IBC transfer channels found on this network.');
    }
...
function getViewDescription(): string {
  const descs: Record<string, string> = {
    ...
    cosmos: t('Your accounts across every chain in the Cosmos registry, starting with Lumen.')
  };
...
function sendTransaction() {
  ...
  sendFeeSchedule.value = null;
  sendAssetDecimals.value = 6;
  showContactPicker.value = false;
  ...
  try {
    ...
    if (isIbcSend.value) {
      ...
      sendParams = {
        ...
        decimals: sendAssetDecimals.value,
        amountText: String(sendForm.value.amount || ''),
        memo: '',
        sourceChannel: selectedIbcChannel.value.channelId,
        sourcePort: selectedIbcChannel.value.portId,
        timeoutSeconds: 600
      };
      // A transfer leaving a Cosmos-panel chain has to be signed and broadcast
      // there, not on Lumen. Without these the handler falls back to its Lumen
      // defaults and the channel id would be read against the wrong chain.
      if (sendAssetContext.value?.rpcEndpoint) {
        const feeGas = DEFAULT_FEE_GAS;
        ...
        sendParams.feeAmount = sendFeeSchedule.value
          ? computeFeeAmount(sendFeeSchedule.value, sendFeeTier.value, feeGas)
          : estimateRemoteFeeAmount(sendAssetContext.value.chainId, feeGas, '2500');
      ...
      if (sendAssetContext.value?.rpcEndpoint) {
        ...
        sendParams.chainId = sendAssetContext.value.chainId;
        sendParams.feeDenom = sendAssetContext.value.feeDenom;
        // The registry's own gas price when the chain publishes one, which is
        // every chain reached through the Cosmos panel. `estimateRemoteFeeAmount`
        // remains the fallback for the two chains hard-coded in ibcChains.ts,
        // whose sends predate this and do not go through a schedule.
        sendParams.feeAmount = sendFeeSchedule.value
          ? computeFeeAmount(sendFeeSchedule.value, sendFeeTier.value, feeGas)
          : estimateRemoteFeeAmount(sendAssetContext.value.chainId, feeGas, '2500');
        sendParams.feeGas = feeGas;
