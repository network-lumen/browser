<template>
  ...
  >
          ...
          <div v-if="filename" class="flex flex-column">
            <UiDetailRow variant="modal" :label="t('File')" :value="filename" />
          </div>
          ...
          >
            ...
            <div v-if="error" class="mt-12px flex flex-column border-radius-12px gap-8px py-12px px-16px border-1-error-a25 bg-error-a08">
              <div class="text-14px txt-weight-light color-text-primary">{{ t('Import failed') }}</div>
          ...
          <template v-else>
            <div class="flex flex-column mt-16px">
              <UiDetailRow variant="modal" :label="t('Wallet')" value-class="color-text-primary text-15px fw-500 mono" :value="details.walletAddress || '—'" />
              <UiDetailRow variant="modal" :label="t('Created')" :value="details.createdAt ? formatDate(details.createdAt) : '—'" />
              <UiDetailRow variant="modal" :label="t('Saved items')" :value="details.filesCount" />
              <UiDetailRow variant="modal" :label="t('Favourites')" :value="details.favCount" />
            </div>
            ...
            >
              <div class="text-14px txt-weight-light color-text-primary">{{ t('Different wallet') }}</div>
              <div class="color-text-secondary text-13px">
                {{ t("This snapshot was created for a different wallet. Importing it will still work, but make sure you're restoring into the right profile.") }}
              </div>
            </div>
