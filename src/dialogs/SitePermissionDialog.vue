<template>
  <UiModal :model-value="modelValue" panel-class="w-min-520px-92vw max-h-100vh-32px" @update:model-value="$emit('deny')">
    <template #header>
      <UiModalHeader :title="t('Permission required')" badge-class="w-32px h-32px bg-fill-blue color-primary" gap-class="gap-10px">
        <template #icon><Shield :size="18" /></template>
      </UiModalHeader>
    </template>
          <UiBanner variant="info">
            <span>
              {{ t('Allow this website to open Lumen action modals?') }}
            </span>
          </UiBanner>

          <div class="border-radius-10px border-default py-10px px-12px">
            <UiDetailRow variant="baseline" :label="t('Site')" :value="siteLabel" label-extra-class="flex-shrink-0" value-class="mono color-text-primary text-right text-13px overflow-wrap-anywhere min-w-0" />
            <UiDetailRow v-if="actionKind" variant="baseline" :label="t('Action')" :value="actionKind" />
          </div>
    <template #footer>
