<template>
  <UiModal :model-value="modelValue" title="Create Proposal" panel-class="w-full max-w-640px" @update:model-value="$emit('update:modelValue', false)">
    ...
    <div class="mb-20px">
      ...
      <UiInput radius-class="border-radius-10px" padding-class="p-14px" :focus-ring="false" type="text" v-model="form.title" placeholder="Enter proposal title..." class="focus-outline-none focus-ring focus-shadow bg-primary" />
    </div>

    <div class="mb-20px">
      <label class="txt-weight-light color-text-primary block text-13px mb-8px">Summary</label>
      <UiInput type="textarea" radius-class="border-radius-10px" padding-class="p-14px" :focus-ring="false" v-model="form.summary" rows="6" placeholder="Describe your proposal in detail..." class="resize-vertical focus-outline-none focus-ring focus-shadow bg-primary"></UiInput>
    </div>
      ...
      <div v-for="draft in actionDrafts" :key="draft.id" class="bg-secondary border-1-light border-radius-10px p-16px mb-12px">
        <div class="flex-align-center gap-10px mb-12px">
          <select v-model="draft.templateId" @change="resetActionDraftValues(draft)" class="flex-1 hover-border-accent cursor-pointer py-8px px-12px border-1 border-radius-8px bg-card color-text-primary text-13px transition-all-02 focus-outline-none focus-border-primary focus-ring focus-shadow">
            <option v-for="tpl in templates" :key="tpl.id" :value="tpl.id">{{ tpl.module }} — {{ tpl.label }}</option>
          </select>
          <UiButton variant="icon" icon-radius-class="border-radius-8px" class="hover-bg-error-a08 hover-color-error size-32px flex-shrink-0" title="Remove action" @click="$emit('remove-action', draft.id)">
            <X :size="16" />
          </UiButton>
        </div>
