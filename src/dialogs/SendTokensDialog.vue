        ...
        <template #icon><Send :size="20" /></template>
          ...
          <UiFormGroup v-if="isIbcSend" required :label="t('IBC route')">
            ...
            <template v-if="selectedIbcChannel || ibcChannelsError" #hint>
              <span v-if="selectedIbcChannel" class="flex-inline-align-center gap-6px">
                <img
                  v-if="selectedIbcChannel.iconUrl"
                  :src="selectedIbcChannel.iconUrl"
                  :alt="selectedIbcChannel.chainLabel || selectedIbcChannel.chainId"
                  class="size-16px border-radius-circle flex-shrink-0 object-fit-contain"
                />
                <span>
                  {{ t('To {chain} via {route}', {
                    chain: selectedIbcChannel.chainLabel || selectedIbcChannel.chainId || t('Unknown chain'),
          ...
          <!-- Only where the chain actually charges gas. Lumen does not, so
               this never appears on a home-chain send. -->
          <UiFormGroup v-if="feeTierOptions.length" :label="t('Network fee')">
            <div class="flex-align-center gap-8px bg-secondary border-radius-10px p-4px flex-wrap-wrap">
              <UiSegmentedButton
                v-for="option in feeTierOptions"
                :key="option.tier"
                :active="form.gasFee === option.tier"
                extra-class="flex-1 flex-justify-center"
                @click="$emit('update:fee-tier', option.tier)"
              >
                <span class="flex flex-column gap-2px text-left">
                  <span>{{ option.label }}</span>
                  <span class="text-12px color-text-tertiary">{{ option.cost }}</span>
                </span>
              </UiSegmentedButton>
            </div>
          </UiFormGroup>
...
import type { FeeTier, IbcChannelOption, SendForm, IbcForm, SendTargetMode } from '../types/walletPage';
...
const props = withDefaults(defineProps<{
  ...
  /**
   * The gas tiers this chain publishes, already priced. Empty on a chain with
   * no fee market - Lumen - where offering a choice would be meaningless.
   */
  feeTierOptions?: { tier: FeeTier; label: string; cost: string }[];
  /** The chosen tier's cost, for the summary. Empty when there is no fee. */
  feeLabel?: string;
}>(), {
  feeTierOptions: () => [],
  feeLabel: '',
...
defineEmits<{
  ...
  (e: 'update:fee-tier', tier: FeeTier): void;
}>();
