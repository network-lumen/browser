<template>
  <UiModal :model-value="modelValue" :title="t('Subscription details')" panel-class="w-full max-w-520px" @update:model-value="$emit('close')">
          ...
          <template v-else>
            <div class="flex flex-column">
              <UiDetailRow variant="modal" :label="t('Gateway')" :value="gatewayLabel" />
              ...
              <UiDetailRow variant="modal" :label="t('Saved')" :value="pinned.length" />
            </div>

            <div class="mt-24px">
              <div class="flex-align-center-justify-space-between gap-8px mb-8px">
                <h4 class="m-0px text-15px fw-500 color-text-primary">{{ t('Usage') }}</h4>
              </div>
              <div
              ...
              >
                <div class="text-14px txt-weight-light color-text-primary">{{ t('Wallet locked') }}</div>
                <div class="color-text-secondary text-13px">
                  {{ t('Unlock your Lumen identity to fetch usage from this cloud.') }}
                </div>
              ...
              <div v-else-if="usage" class="flex flex-column">
                <UiDetailRow variant="modal" :label="t('Quota')" :value="
                    ...
                    usage.plan?.quotaBytesTotal != null
                      ...
                      : '-'
                  " />
                <UiDetailRow variant="modal" :label="t('Used')" :value="
                    ...
                    usage.plan?.quotaBytesUsed != null
                      ...
                      : '-'
                  " />
                <UiDetailRow variant="modal" :label="t('Bandwidth')" :value="bandwidthUsed" />
                <UiDetailRow variant="modal" :label="t('Roots')" :value="usage.usage?.roots_total ?? usage.usage?.rootsTotal ?? '-'" />
              </div>
            </div>
          </template>
  </UiModal>
