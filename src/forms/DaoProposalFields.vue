<template>
  ...
  <div class="flex flex-column gap-12px">
    <div class="gap-12px grid grid-cols-2-minmax0">
      <UiFormField :label="t('Action')">
        <select v-model="form.kind" class="w-full border-radius-12px color-text-primary text-15px line-height-12 border-1 bg-secondary py-8px px-10px focus-outline-none focus-border-primary focus-ring focus-shadow">
          <option value="validate">{{ t('Validate release') }}</option>
          <option value="reject">{{ t('Reject release') }}</option>
        </select>
      </UiFormField>
      <UiFormField :label="t('Deposit (LMN)')">
    ...
    <UiFormField v-if="form.kind === 'reject'" :label="t('Reason (optional)')">
      <UiInput type="textarea" bg-class="bg-secondary" radius-class="border-radius-12px" font-size-class="text-15px line-height-12" padding-class="py-8px px-10px" :focus-ring="false" v-model="form.reason" rows="3" :placeholder="t('Why should this release be rejected?')" class="focus-outline-none focus-ring focus-shadow" />
    </UiFormField>
  </div>
  </div>
</template>
