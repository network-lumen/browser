...
function resolveKuboBin() {
  ...
  // Whatever `ipfs` means on this machine, which is a gamble worth naming: the
  // repository format is tied to the Kubo version, so an unrelated install -
  // IPFS Desktop ships its own - can be too old to open a repo this app has
  // already migrated, and then nothing starts. Reached only when the bundled
  // module could not be loaded at all, which is a packaging fault.
  console.warn('[electron][ipfs] bundled kubo not resolvable, falling back to `ipfs` on PATH');
  return 'ipfs';
}
 ...
 */
/**
 * Set when the repo on disk is newer than the Kubo shipped with this build.
 *
 * Nothing in the app can fix that: Kubo refuses the repo, the daemon exits, the
 * API never opens, and every caller sees `fetch failed` forever. The two numbers
 * that explain it are known here and were only ever written to a log line, so a
 * user watching the app retry had no way to learn that a newer build had
 * migrated the repository out from under this one.
 *
 * It happens for real between a development checkout and an installed build:
 * both use %APPDATA%/lumen/ipfs, and whichever ships the newer Kubo migrates it
 * forward for good.
 */
let repoTooNewFor = null;

function getRepoCompatibilityError() {
  return repoTooNewFor;
}
...
function migrateIpfsRepoIfNeeded(bin, repoPath) {
  ...
  if (plan !== 'newer') repoTooNewFor = null;
  if (plan === 'none') return;
  if (plan === 'newer') {
    console.warn('[electron][ipfs] repo is newer than this Kubo', { current, expected });
    repoTooNewFor = { repoVersion: current, binaryVersion: expected };
    return;
  }
...
async function checkIpfsStatus(retries = 3, delay = 1000) {
  // Answered before the first fetch, and without retrying: no number of
  // attempts opens an API whose daemon refused the repository and exited.
  // Retrying here is what turned a knowable incompatibility into an app that
  // sat on "fetch failed".
  if (repoTooNewFor) {
    return {
      ok: false,
      error: 'repo_newer_than_binary',
      repoVersion: repoTooNewFor.repoVersion,
      binaryVersion: repoTooNewFor.binaryVersion,
    };
  }
